...
namespace Rawler.Condition
{
    public class Equal : RawlerBase
    {
        ...
        public bool Check
        {
            ...
            {
                var t = GetText();
                if(dic.ContainsKey(t.ToCharArray().First()))
                {
                    return  dic[t.ToCharArray().First()].Any(n => n.Equals(t));
                }
                else
                {
                    return false;
            ...
            if (createOnce == false)
            {
                if (EqualDataTree != null)
                {
                    ...
                    foreach (var item in EqualDataTree.Texts)
                    {
                        ...
                        {
                            if (dic.ContainsKey(item.Value.ToCharArray().First()))
                            {
                                dic[item.Value.ToCharArray().First()].Add(item.Value);
                            }
                            else
                            {
                                dic.Add(item.Value.ToCharArray().First(), new List<string>() { item.Value });
                            }
                        }
                    }
                }
                if (equalArray != null && equalArray.Length > 0)
                {
                    ...
                    {
                        if (dic.ContainsKey(item.ToCharArray().First()))
                        {
                            dic[item.ToCharArray().First()].Add(item);
                        }
                        else
                        {
                            dic.Add(item.ToCharArray().First(), new List<string>() { item });
                        }
                    }
                }
                createOnce = true;
